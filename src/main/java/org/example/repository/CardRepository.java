...
public class CardRepository {
    ...
    public int payFromCard(String cardNum, Double amount) {
        try (Connection connection = DataBase.getConnection()) {
            PreparedStatement ps = connection.prepareStatement("update card set balance = ? where card_number = ?");
            ps.setDouble(1, amount);
            ps.setString(2, cardNum);
    ...
    public Card getCardByProfilePhoneOne(String phone) {
        try {
            Connection connection = DataBase.getConnection();
            String sql = "select * from card where visible = true and phone = '" + phone + "';";
            Statement statement = connection.createStatement();
            ...
            if (resultSet.next()) {
                ...
                card1.setStatus(GeneralStatus.valueOf(status));
                card1.setPhone(phone);
                card1.setCreatedDate(createdDate);
            }
            return card1;
        } catch (SQLException sqlException) {
            sqlException.printStackTrace();
    ...
    public int payCorporativeCard(String corCardNumber, Double amount) {
        try (Connection connection = DataBase.getConnection()) {
            PreparedStatement ps = connection.prepareStatement("update card set balance = ? where card_number = ?");
            ps.setDouble(1, amount);
            ps.setString(2, corCardNumber);
            return ps.executeUpdate();
        } catch (SQLException sqlException) {
            sqlException.printStackTrace();
        }
    ...
    public ResultSet getCorporativeCadrBalance() {
        try {
            Connection connection = DataBase.getConnection();
            String sql = "select balance from card where card_number = '5555'";
            Statement statement = connection.createStatement();
            ResultSet resultSet = statement.executeQuery(sql);
            if (resultSet.next()) {
