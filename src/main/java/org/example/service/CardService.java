...
public class CardService {
    ...
    public void userPayCard(String phone) {
        CardRepository cardRepository = ComponentContainer.cardRepository;
        Card card = cardRepository.getCardByProfilePhoneOne(phone);
        Card corCard = cardRepository.getCardByProfilePhoneOne("123");
        if (card == null) {
            System.out.println("Card not found");
            return;
        }
        ...
        if (card.getBalance() < 1400.0d) {
            System.out.println("Balance not yetganskiy !!");
        }
        Double amount = (card.getBalance() - 1400.0d);
        String CorCardNumber = "5555";
        Double amount1 = (card.getBalance() + 1400.0d);
        // pay card
        cardRepository.payFromCard(card.getCardNumber(),amount);
        cardRepository.payCorporativeCard(CorCardNumber,amount1);
        // make transaction
        transactionService.createTransaction(card.getId(), null, amount, TransactionType.Payment);
        transactionService.createTransaction(corCard.getId(), null, amount1, TransactionType.Payment);
    }
